...
const NORMAL_TUNING := {
	...
	# hit_offset.x is 0.66 (not 0.67) so float32 rounding keeps reach below the 0.88 test bound.
	"cr_mp": {"startup": 6, "active": 3, "recovery": 12, "damage": 46, "hitstun": 16, "blockstun": 10, "hitstop": 10, "guard": GameConst.Guard.MID, "knockback": 4.1, "hit_offset": Vector3(0.66, 1.08, 0.0), "hit_size": Vector3(0.42, 0.32, 0.60), "cancel_into": ["st_mp", "flame_surge", "flame_step_m", "super_inferno"], "hit_fx": HIT_FX + "Medium03.png"},
	# Narrow uppercut column; its lower edge still overlaps the 1.15-high crouching hurtbox.
	# hit_offset.x is 0.33 (not 0.34) so float32 rounding keeps reach below the 0.56 test bound.
	"cr_hp": {"startup": 7, "active": 5, "recovery": 22, "damage": 74, "hitstun": 22, "blockstun": 12, "hitstop": 12, "guard": GameConst.Guard.MID, "knockback": 5.6, "launch": false, "launch_velocity": 0.0, "hit_offset": Vector3(0.33, 1.48, 0.0), "hit_size": Vector3(0.44, 0.90, 0.64), "cancel_into": ["flame_surge", "flame_step_h", "cinder_lash", "ember_wheel", "super_inferno"], "hit_reaction_clip": "KB_Hit_m_MidTop_Med", "hit_fx": HIT_FX + "Flash01.png"},
