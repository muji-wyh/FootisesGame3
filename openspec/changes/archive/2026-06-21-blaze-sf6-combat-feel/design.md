...
- *Hitstop handling (critical):* `poll_input()` keeps pushing frames during the impact `hitstop` freeze (the attacker is not `_is_locked_out`), while `advance()`/`_step_attack` are skipped. The buffer must therefore **not age during hitstop**: measure `CANCEL_BUFFER` in *advancing* ticks (where `hitstop == 0`), or latch a pending-cancel at `mark_connected` and re-evaluate each advancing tick. Otherwise a pre-pressed/mashed follow-up ages out across a 6–10f freeze and the buffered-cancel scenarios fail for early presses.
- *Discovered during implementation (the actual root cause):* every move's `cancel_into` was **empty at runtime**. `CharacterLibrary._move()` filled moves via `Object.set("cancel_into", value)`, but `cancel_into` is typed `Array[String]` and Godot silently drops a generic `Array` source on `set()` — so the cancel lattice never existed, and no cancel could ever fire regardless of the input path. Fixed by using `Array.assign()` for typed-array properties (`cancel_into`, `motion`) in `_move()`. The buffering/first-contact changes above are still required for the inputs to register; this was the additional, silent blocker.
...
- *State wiring:* add `DRIVE_RUSH` to the `advance()` dispatch; keep it OUT of `_is_locked_out`, `_can_block`, and `_is_actionable` (not blockable, not neutral-actionable); `update_facing` treats it as locked; `_on_landed` is unaffected (grounded). `reset_for_round()` must clear `drive_rush_pending`.
...
- *DRC eligibility:* a normal is DRC-cancellable **iff it is already special-cancellable** (non-empty `cancel_into`); no separate `drive_rush_cancellable` flag is introduced. DRC is permitted on hit and on block.

### D6 — All Blaze balance stays in `CharacterLibrary` data; engine only gains generic mechanics
Frame-data nudges for true combos, Drive Rush move parameters, and `cancel_into` routes live in `CharacterLibrary.gd`/`MoveData`. The engine gains only generic, character-agnostic mechanics (buffering, gauge, Drive Rush state). New `MoveData` fields (e.g., `rises`/`rise_height`) are optional with safe defaults; DRC eligibility reuses the existing `cancel_into` rather than adding a flag (see D5).
...
- *Hitbox tuning (watch-out):* because the arc raises `position.y`, the uppercut's high `hit_offset.y` (1.6) can ride above a standing hurtbox and **whiff a grounded opponent** (regressing it as a combo ender). Start the rise at/after the active window opens and/or lower `hit_offset.y` + enlarge `hit_size.y`, verified by a headless assert that the uppercut connects on a grounded dummy (not only as an anti-air).
...
- **Raw Drive Rush replacing the neutral movement dash could waste Drive or feel bad.** → Only convert to RDR when affordable; preserve the ordinary dash as the fallback when Drive is empty. Per user direction `→→` is RDR with no separate gate; revisit after playtest (a future Burnout/empty state naturally restores free dashing).
